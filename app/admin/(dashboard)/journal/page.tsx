...
export default async function AdminJournalPage({ searchParams }: { searchParams?: { siteId?: string; locale?: string } }) {
  ...
  return (
    <ContentEditor sites={visibleSites} selectedSiteId={selectedSite?.id || ''} selectedLocale={searchParams?.locale || selectedSite?.defaultLocale || 'en'}
      fileFilter="journal" titleOverride="Journal Posts" basePath="/admin/journal" />
  );
}
