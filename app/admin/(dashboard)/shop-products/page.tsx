...
export default async function AdminShopProductsPage({ searchParams }: { searchParams?: { siteId?: string; locale?: string } }) {
  ...
  return (
    <ContentEditor sites={visibleSites} selectedSiteId={selectedSite?.id || ''} selectedLocale={searchParams?.locale || selectedSite?.defaultLocale || 'en'}
      fileFilter="shopProducts" titleOverride="Shop Products" basePath="/admin/shop-products" />
  );
}
