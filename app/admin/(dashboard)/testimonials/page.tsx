...
export default async function AdminTestimonialsPage({ searchParams }: { searchParams?: { siteId?: string; locale?: string } }) {
  ...
  return (
    <ContentEditor sites={visibleSites} selectedSiteId={selectedSite?.id || ''} selectedLocale={searchParams?.locale || selectedSite?.defaultLocale || 'en'}
      fileFilter="testimonials" titleOverride="Client Testimonials" basePath="/admin/testimonials" />
  );
}
